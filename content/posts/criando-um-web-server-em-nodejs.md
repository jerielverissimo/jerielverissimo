...
Quando você visualiza uma página web no seu browser, seu computador está fazendo 
uma requisição para um outro computador (🌈 Internet), a resposta desta requisisção normalmente é uma página HTML.
Na real as coisas não são tão simples assim, tem bastante coisa que rola no meio do caminho para isso funcionar, 
mas normalmente quem devolve essa resposta para seu browser é um web server.
...
como um ponto de partida que explica como a web funciona.
...
e o nodejs já traz em sua biblioteca padrão um módulo chamado ***http*** que adivinha o que ele faz? Isso mesmo, abstrai pra gente as funcionalidades desse protocolo.
...
Então passamos para ele um endereço e uma porta, que em nosso caso como ele vai rodar em nossa própria máquina
vamos passar ***localhost*** como o endereço e ***8080*** como porta.
...
Bom voltando para o nosso arquivo server.js, cria logo abaixo da importação do modulo http o endereço e porta para usarmos depois.
...
Como no nosso caso queremos devolver uma reposta, vamos utilizar o objeto de *response*.
...
Só com essa função não conseguimos fazer muita coisa, quem de fato vai fazer a mágica é a lib http do nodejs, basta a gente
criar um servidor usando a lib passando nossa função que acabamos de criar como argumento, que será usada toda vez que esse servidor
...
Usando nosso exemplo, altera a função listener que criamos para adicionar na resposta um ***header***  que vai indicar
que estamos enviando como resposta um arquivo HTML, e muda o conteúdo também para um html válido.
...
Um outro formato que é bem comum é o **JSON**, que é bastante utilzando quando se envolve ***APIs***.
...
continuo de forma **assincrona** para que, por exemplo, uma requição que demore mais para ser respondida não bloqueia o servidor de receber
...
Como em requisição do tipo **POST** possui um ***body*** como conteúdo, vamos precisar primeiro montar esse body a medida que ele vai chegando.
...
Criamos uma variável chamada *body* para receber o conteúdo da requisição, e passamos ela na callback para ir incrementando seu conteudo.
...
Quando não tem mais dados para receber, um evento do tipo **end** é emitido e com isso passamos pra ele uma callback
que vai apenas printar o conteúdo que recebemos ou seja, o body da requisição **POST** 
e devolver uma resposta para o cliente, ou seja para quem fez esta requisição.

Para testar faça agora uma requisição do tipo **POST**, nesse caso vai receber a mensagem: 'Request received'
...
Por ultimo, quando lidamos com um servidor web é bem comum fazermos requisições via **rotas** assim podemos dividir nosso
...
const post = function (req, res) {
  ...
  // quando todos os pedaços da requisiçã chegaram :)
  req.on('end', () => {
    console.log(body); // mostra o conteúdo que recebemos da requisição
    res.writeHead(200, { 'Content-Type': 'application/json' });
    res.end('Request received'); // devolve resposta para o cliente
...
Agora no nosso listener, vamos olhar para a url da requisição para ser usado como rota.
...
Agora para listar os usuários adcionados simplesmente criamos uma função chamada get que devolve o array de usuários.
...
E assim terminamos esse longo artigo, espero que tenha aprendido algo com ele, caso queira ver o arquivo final criei este [gist](https://gist.github.com/jerielverissimo/c84d24fb66bf813d056d8cef0aff270e).
